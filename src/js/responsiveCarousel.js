	...
	e.fn.carousel = function(t) {
		...
		return e(this).each(function() {
			...
			r.init = function() {
				...
				e(r)
					.find('.crsl-item')
					.css({ position: 'relative', float: 'left', overflow: 'hidden', height: 'auto' })
				e(r)
				...
				e(r)
					.find('.crsl-item iframe')
					.attr({ width: '100%' })
				if (n.carousel)
					e(r)
						.find('.crsl-item:first-child')
						.addClass(n.itemClassActive)
			...
			r.config = function() {
				...
				if (n.itemWidth <= n.itemMinWidth) {
					n.visible =
						Math.floor((e(r).outerWidth() - n.itemMargin * (n.visible - 1)) / n.itemMinWidth) === 1
					...
					n.itemWidth =
						n.visible === 1
							...
							: Math.floor((e(r).outerWidth() - n.itemMargin * (n.visible - 1)) / n.visible)
				} else {
				...
				if (n.carousel) {
					...
					r.wrapMargin = r.wrapMarginDefault =
						n.infinite && n.visible < n.total ? parseInt((n.itemWidth + n.itemMargin) * -1, 10) : 0
					if (
						n.infinite &&
						n.visible < n.total &&
						e(r)
			...
			r.testNext = function() {
				return (
					(!n.infinite && r.wrapWidth >= (n.itemWidth + n.itemMargin) * (n.visible + 1) - r.wrapMargin) ||
					n.infinite
				)
			}
			r.previous = function() {
			...
			e(window).on('mouseleave', function(t) {
				...
				if (
					...
					e(s)
						.parents('.crsl-items')
						.data('navigation') === e(r).data('navigation')
				) {
					i = true
				} else {
					i = false
				...
				e(r)
					.on('touchstart', function(t) {
						...
						e('.touching').on('touchmove', function(e) {
							...
							if (
								Math.abs(parseInt(n.endCoords.pageX - n.startCoords.pageX, 10)) >
